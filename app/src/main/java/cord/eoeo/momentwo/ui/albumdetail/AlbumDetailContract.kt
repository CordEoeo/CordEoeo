...
class AlbumDetailContract {
    data class State(
        val albumItem: AlbumItem = AlbumItem(-1, "", "", "", 0),
        ...
        val selectedMemberIds: List<Int> = emptyList(),
    ...
    sealed interface Event : UiEvent {
        ...
        data class OnChangeFriendSelected(
            val isSelected: Boolean,
            val friendItem: FriendItem,
        ) : Event

        data class OnChangeSubAlbumSelected(
            val isSelected: Boolean,
            val subAlbumId: Int,
        ) : Event

        data class OnChangeMemberSelected(
            val isSelected: Boolean,
            val memberId: Int,
        ) : Event
        ...
        data class OnOpenTextFieldDialog(
            val index: Int,
        ) : Event
        ...
        data class OnChangeIsInChangeImage(
            val isInChangeImage: Boolean,
        ) : Event
        ...
        data class OnSubAlbumEvents(
            val subAlbumEvents: SubAlbumEvents,
        ) : Event

        data class OnMemberEvents(
            val memberEvents: MemberEvents,
        ) : Event

        data class OnAlbumSettingEvents(
            val albumSettingEvents: AlbumSettingEvents,
        ) : Event
    ...
    sealed interface MemberEvents {
        ...
        data class KickMembers(
            val kickMemberIds: List<Int>,
        ) : MemberEvents

        data class AssignAdmin(
            val nickname: String,
        ) : MemberEvents

        data class EditPermissions(
            val editMembers: List<MemberItem>,
        ) : MemberEvents
    }
