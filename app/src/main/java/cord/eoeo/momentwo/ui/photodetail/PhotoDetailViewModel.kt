    ...
    override fun handleEvent(newEvent: PhotoDetailContract.Event) {
        when (newEvent) {
            ...
            is PhotoDetailContract.Event.OnGetLikeCount -> {
                viewModelScope.launch {
                    with(uiState.value) {
                        likeRepository.getLikeCount(albumId, photoId)
                            .onSuccess { count ->
            ...
            is PhotoDetailContract.Event.OnGetDescription -> {
                viewModelScope.launch {
                    with(uiState.value) {
                        descriptionRepository.getDescription(albumId, photoId)
                            .onSuccess { desc ->
            ...
            is PhotoDetailContract.Event.OnWriteDescription -> {
                viewModelScope.launch {
                    with(uiState.value) {
                        ...
                        descriptionRepository.writeDescription(albumId, photoId, descString)
                            .onSuccess {
            ...
            is PhotoDetailContract.Event.OnEditDescription -> {
                viewModelScope.launch {
                    with(uiState.value) {
                        ...
                        descriptionRepository.editDescription(albumId, photoId, descString)
                            .onSuccess {
                                setEvent(PhotoDetailContract.Event.OnGetDescription)
                            }.onFailure { exception ->
                                // TODO
            ...
            is PhotoDetailContract.Event.OnDeleteDescription -> {
                viewModelScope.launch {
                    with(uiState.value) {
                        descriptionRepository.deleteDescription(albumId, photoId)
                            .onSuccess {
                                setState(copy(descriptionItem = DescriptionItem("", "", "", "")))
                            }.onFailure { exception ->
                                // TODO
            ...
            is PhotoDetailContract.Event.OnWriteComment -> {
                viewModelScope.launch {
                    with(uiState.value) {
                        commentRepository.writeComment(albumId, photoId, newEvent.comment)
                            .onSuccess {
            ...
            is PhotoDetailContract.Event.OnEditComment -> {
                viewModelScope.launch {
                    with(uiState.value) {
                        commentRepository.editComment(albumId, newEvent.commentId, newEvent.comment)
                            .onSuccess {
                                setEffect { PhotoDetailContract.Effect.RefreshCommentPagingData }
                            }.onFailure { exception ->
                                // TODO
            ...
            is PhotoDetailContract.Event.OnDeleteComment -> {
                viewModelScope.launch {
                    with(uiState.value) {
                        commentRepository.deleteComment(albumId, newEvent.commentId)
                            .onSuccess {
                                setEffect { PhotoDetailContract.Effect.RefreshCommentPagingData }
                            }.onFailure { exception ->
                                // TODO
